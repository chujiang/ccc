<#if question.type == "T" || question.type == "A">
	...
	<tr id="${question.type }_${question.id}_question_content" questiontype="${question.type }">
		<td>
			<#if question.type == "A">
				<textarea readonly name="question_${question.id?c}" style="margin-left:35px;text-align:left;" cols="50" rows="5" onChange=""></textarea>
			</#if>
		</td>
	</tr>
<#else>
						...
						<#list question.questionAnswers as answer >
							<#if answer??>
								...
								<#if question.type == "C">
									...
									<input type="checkbox" onClick="" disabled="disabled" name="question_${question.id?c}" seq="${ answer.itemSeq }" value="${ answer.id?c }" />
								...
								<#if question.type == "R" || question.type == "B">
									<input type="hidden" name="question_${question.id?c}_id" value="${ answer.questionId?c }" />
									<input type="radio" disabled="disabled" onClick="" name="question_${question.id?c}" seq="${answer.itemSeq }" value="${answer.id?c}"/>
