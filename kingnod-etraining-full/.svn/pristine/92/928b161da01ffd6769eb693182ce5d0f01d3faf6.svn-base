			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				if (openFlag) {
					...
					html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level,"' treeNode", consts.id.A," onclick=\"", (node.click || ''),
						...
						"'");
					if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle)) {html.push("title='", null!=node[titleKey]?node[titleKey].replace(/'/g,"&#39;"):"aaaaaa","'");}
					html.push(">");
...
(function($){
	...
	_view = {
		...
		editNode: function(setting, node) {
			...
			$("#" + node.tId + consts.id.SPAN).html("<input type=text style='width:" + node.width + "px' class='rename' id='" + node.tId + consts.id.INPUT + "' treeNode" + consts.id.INPUT + " >");
