...
public class HtmlBuilder {
    ...
    public HtmlBuilder attr(String attr,String attrValue) {
    	if (StringUtils.isNotBlank(attrValue) && StringUtils.isNotBlank(attr)) {
    		append(" ").append(attr).append("=\"").append(attrValue).append("\"");
    	}
    	return this;
    }
    
    public HtmlBuilder attrs(Object...attrs) {
    	Map  map = MapUtils.mapByAarray(attrs);
    	return attrs(map);
    }
    
    public HtmlBuilder attrs(Map map) {
    	if (MapUtils.isNotEmpty(map)) {
    		Set<Entry> eSet = map.entrySet();
    		
    		for (Entry  entry : eSet) {
    			
    			String attrname = ObjectUtils.toString(entry.getKey());
    			String attrvalue = ObjectUtils.toString(entry.getValue());
